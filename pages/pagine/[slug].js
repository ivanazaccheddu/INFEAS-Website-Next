...
import paginePageInitialPropsTqXqResource from '../../resources/pagine-page-initial-props-tq_xq'
import paginePageInitialPathsTqJIResource from '../../resources/pagine-page-initial-paths-tq_j-i'
...
export async function getStaticProps(context) {
  try {
    const response = await paginePageInitialPropsTqXqResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await paginePageInitialPathsTqJIResource({})
    return {
