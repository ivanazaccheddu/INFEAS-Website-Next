...
import paginePageInitialPropsTqMsResource from '../../resources/pagine-page-initial-props-tq_ms'
import paginePageInitialPathsTq9mResource from '../../resources/pagine-page-initial-paths-tq_9m'
...
export async function getStaticProps(context) {
  try {
    const response = await paginePageInitialPropsTqMsResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await paginePageInitialPathsTq9mResource({})
    return {
