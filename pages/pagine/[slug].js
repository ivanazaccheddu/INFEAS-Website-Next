...
import paginePageInitialPropsTqEyResource from '../../resources/pagine-page-initial-props-tq_ey'
import paginePageInitialPathsTqLOResource from '../../resources/pagine-page-initial-paths-tq_l-o'
...
export async function getStaticProps(context) {
  try {
    const response = await paginePageInitialPropsTqEyResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await paginePageInitialPathsTqLOResource({})
    return {
