...
import eventiPageInitialPropsTqFCResource from '../../resources/eventi-page-initial-props-tq_f-c'
import eventiPageInitialPathsTqHmResource from '../../resources/eventi-page-initial-paths-tq_hm'
...
export async function getStaticProps(context) {
  try {
    const response = await eventiPageInitialPropsTqFCResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await eventiPageInitialPathsTqHmResource({})
    return {
