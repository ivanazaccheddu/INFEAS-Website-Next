...
import eventiPageInitialPropsTqGiResource from '../../resources/eventi-page-initial-props-tq_gi'
import eventiPageInitialPathsTqJ2Resource from '../../resources/eventi-page-initial-paths-tq_j2'
...
export async function getStaticProps(context) {
  try {
    const response = await eventiPageInitialPropsTqGiResource({
      ...context?.params,
    })
    if (!response?.data?.[0]) {
      return {
...
export async function getStaticPaths() {
  try {
    const response = await eventiPageInitialPathsTqJ2Resource({})
    return {
